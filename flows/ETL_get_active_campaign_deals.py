...
def extract_active_campaign_deals_closed() -> list[dict]:
    ...
    while True:
        print(f'Iteration: {offset / 100}')
        ac_response = requests.get(
            url=f'{deals_url}?limit=100&offset={offset}&filters[updated_before]={current_date_1day_add.strftime("%Y-%m-%d")}&filters[updated_after]={current_date_1day_sub.strftime("%Y-%m-%d")}&filters[stage]=99&filters[status]=0',
            headers={
                'Accept': 'application/json',
                'Api-Token': ac_token.get()
            }
